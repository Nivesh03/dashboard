...
import { IntegrationValidator, ValidationResult } from '@/lib/integration-validator';
  ...
  const runTests = async () => {
    ...
    try {
      ...
      testResults.push({
        ...
        duration: validationResults.reduce((sum, r) => sum + (r.duration || 0), 0)
      });
    ...
    try {
      ...
      testResults.push({
        name: 'Data Hooks Integration',
        status: 'fail',
        message: `Data hooks error: ${error instanceof Error ? error.message : 'Unknown error'}`
      });
    }
    ...
    try {
      ...
      testResults.push({
        name: 'Component Rendering',
        status: 'fail',
        message: `Component rendering error: ${error instanceof Error ? error.message : 'Unknown error'}`
      });
    }
    ...
    try {
      ...
      if (navLinks.length >= 4) { // Overview, Analytics, Data, Reports
        testResults.push({
      ...
      testResults.push({
        name: 'Navigation Flow',
        status: 'fail',
        message: `Navigation test error: ${error instanceof Error ? error.message : 'Unknown error'}`
      });
    }
    ...
    try {
      ...
      testResults.push({
        name: 'Theme System',
        status: 'fail',
        message: `Theme system error: ${error instanceof Error ? error.message : 'Unknown error'}`
      });
    }
    ...
    try {
      ...
      testResults.push({
        name: 'Responsive Design',
        status: 'fail',
        message: `Responsive design test error: ${error instanceof Error ? error.message : 'Unknown error'}`
      });
    }
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        ...
        {tests.length > 0 && (
          <Alert className={overallStatus === 'pass' ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}>
            <AlertDescription>
              {overallStatus === 'pass' 
